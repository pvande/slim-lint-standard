...
module SlimLint
  # Checks for consistent quote usage in HTML attribute values.
  #
  # Since the fork's parser carries column-level position on every node,
  # violations are reported at the exact column of the offending attrs node.
  class Linter::QuoteConsistency < Linter
    ...
    on [:html, :attrs] do |node|
      ...
      if enforced_style == :single_quotes && double_quotes.any?
        report_lint(node, format(MSG, "Use single quotes for attribute values (')"))
      elsif enforced_style == :double_quotes && single_quotes.any?
        report_lint(node, format(MSG, 'Use double quotes for attribute values (")'))
      end
    end
